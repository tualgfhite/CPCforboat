...
## PyTorch implementation of cpcK2, cpcK5, cpcK6, speaker classifier models
# cpcK2: base model from the paper 'Representation Learning with Contrastive Predictive Coding'
# cpcK5: cpcK2 with a different decoder
# cpcK6: cpcK2 with a shared encoder and double decoders
# boatClassifier: a simple NN for speaker classification

class cpc6(nn.Module):
    ''' cpcK2 with double decoder and a shared encoder '''
    def __init__(self, timestep, batch_size, seq_len):

        super(cpc6, self).__init__()
...
class cpc5(nn.Module):
    ''' cpcK2 with a different decoder '''
    def __init__(self, timestep, batch_size, seq_len):

        super(cpc5, self).__init__()
...
class cpc2(nn.Module):
    def __init__(self, timestep, batch_size, seq_len):

        super(cpc2, self).__init__()

        self.batch_size = batch_size
        self.seq_len = seq_len
        self.timestep = timestep
        self.encoder = nn.Sequential( # downsampling factor = 160
            nn.Conv1d(1, 512, kernel_size=8, stride=4, padding=3, bias=False),
            nn.BatchNorm1d(512),
            nn.ReLU(inplace=True),
            nn.Conv1d(512, 512, kernel_size=8, stride=4, padding=2, bias=False),
            nn.BatchNorm1d(512),
    ...
    def forward(self, x, hidden):
        batch = x.size()[0]
        t_samples = torch.randint(int(self.seq_len/128-self.timestep), size=(1,)).long() # randomly pick time stamps
        # input sequence is N*C*L, e.g. 8*1*20480
        z = self.encoder(x)
        # encoded sequence is N*C*L, e.g. 8*512*128
        # reshape to N*L*C for GRU, e.g. 8*128*512
...
class boatClassifier(nn.Module):
    ...
    def __init__(self, boat_num):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Linear(512, boat_num)
            #nn.Linear(256, boat_num)
        )
