...
from src.data_reader.dataset import RawDataset
from src.logger import setup_logs
from src.model.model import cpc2,cpc5,cpc6
from src.training import train, result
...
def main():
    ...
    model = cpc2(args.timestep, args.batch_size, args.audio_window).to(device)
    # model = cpc5(args.timestep, args.batch_size, args.audio_window).to(device)
    # model = cpc6(args.timestep, args.batch_size, args.audio_window).to(device)
    ## Loading the dataset
    params = {'num_workers': 4,
              'pin_memory': False} if use_cuda else {}
    #加载dataset loader model
    logger.info('===> loading train, test dataset')
    training_set = RawDataset(args.trainset)
    ...
    test_set = RawDataset(args.testset)
    # test_set = ReverseRawDataset(args.test_raw, args.test_list, args.audio_window)
    # test_set = RawXXreverseDataset(args.test_raw, args.test_list, args.audio_window)
    train_loader = data.DataLoader(training_set, batch_size=args.batch_size, shuffle=True,
                                   **params)
    test_loader = data.DataLoader(test_set, batch_size=args.batch_size, shuffle=False,
                                        **params)
    #设置warmup和requires_grad
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # val_acc, val_loss = testXXreverse(args, model, device, test_loader, args.batch_size)
    ...
    logger.info("################## Success #########################")
    logger.info("Total time: %s" % (end_global_timer - global_timer))
